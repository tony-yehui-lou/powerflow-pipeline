...
INTERNAL_PREFIX = ".powerflow-"  # marks working directories that are never themselves scans
STAGING_PREFIX = f"{INTERNAL_PREFIX}staging-"
BACKUP_PREFIX = f"{INTERNAL_PREFIX}backup-"
...
def create_staging_root(anchor: Path) -> Path:
    ...
    return Path(tempfile.mkdtemp(prefix=STAGING_PREFIX, dir=anchor.parent))
    ...
    try:
        for staged, destination in replacements:
            ...
            if destination.exists():
                backup = (
                    destination.parent / f"{BACKUP_PREFIX}{uuid.uuid4().hex}-{destination.name}"
                )
